...
* アプリケーションの実行例
  ...
  ```text
  java -jar dept-tree.jar --root "com.example.Hoge" --analyze ./target/classes/**/*.class --analyze ./libs/*.jar --depth 20 --marker ./maker.txt --exclude ./exclude.txt
  ```
...
* メソッドの出現順でツリー化する
  ...
  * 例外処理部のメソッドは呼び出しを追跡する
* メソッド呼び出しツリーはログに出力
  * ログは仕様定義のログ出力で設計
  * ログファイル名は `--root` で指定したクラスのFQDNに `.log` を付与した名前とする
* 引数 (` --exclude` ) で指定するテキストファイルを読み、このファイルに記録されたパッケージ名を "除外するパッケージ" に加える
* 引数 ( `--marker` ) で指定するテキストファイルを読み、このファイルの記録されたクラスのFQDNがツリーに現れた場合、次の処理を行う
  * 出現回数を数える
...
* 任意の引数
* 追跡を除外するパッケージ名を記録したテキストファイルのファイルパス
  * このファイルに記録したパッケージ名は "除外するパッケージ" に加える
  * パッケージ名が "." で終わる場合、先頭一致する場合に除外する
  * パッケージ名が "." で終わらない場合、完全一致する場合に除外する
* テキストファイルは以下のフォーマット
  * 1行に1パッケージ名
  * 改行コードはLF
  * 例
    ```text
    co.jp.example.exclude.
    ```
* 引数に指定されたファイルが存在しない場合常終了とする
...
* `-a/--analyze` で指定する解析対象から除外するパッケージについて、Java標準API および `org`, `io` から始まるパッケージを解析の対象外とする
...
* `-e/--exclude` で指定するテキストファイルに 追加する "除外パッケージ名" が記録されている場合、そのパッケージ名を除外対象に加える
...
| 追加の除外パッケージ名 | -e | --exclude | 任意 | 除外するパッケージ名が記録されたテキストファイルのファイルパス | 不可 | なし |
| 解析深度       | -d | --depth     | 任意 | 1から100の数値                   | 不可 | 100   |
...
* #2) `--analyze` で指定した解析対象ファイルを読み込む
  * 解析対象ファイルにjarファイルがある場合、jarファイルからclassファイルを一時ディレクトリへ抽出する
  * 以後jarファイルは解析対象から外し、一時ディレクトリに抽出したclassファイルを対象とする
  * 読み込んだclassファイルの総数をログにinfoレベルで出力する
* #3) 解析開始クラスが解析対象classに含まれるかチェックする
  * 解析開始クラスが含まれるclassファイルをログにinfoレベルで出力する
* #4) 解析開始クラスにpublic/protected/package-privateスコープのメソッドがあるかチェックする
  * 見つかったメソッド名をログにinfoレベルで出力する
* #5) `--exclude` が指定されている場合、この引数で指定するテキストファイルに記録されたパッケージ名を "除外するパッケージ" に加える
* #6) 解析開始クラスに存在するpublic/protected/package-privateスコープのメソッドを起点にしてメソッドの呼び出しを解析しメソッド呼び出しのツリー情報を生成する
* #7) `--marker` が指定されている場合、この引数で指定するテキストファイルに記録されたクラスのFQDNがツリーに出現する回数を数える
* #8) 生成した呼び出しのツリーの情報をログへ出力する
